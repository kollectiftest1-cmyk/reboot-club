...
from core.models import AuditLog, Club, Deposit, Invitation, Loan, LoanFunding, Membership, User
...
class PermissionTests(APITestCase):
    ...
    def test_admin_can_operate_for_offline_clients(self):
        ...
        # Le placement reste en attente : il ne finance rien avant validation admin.
        funding = LoanFunding.objects.get(loan=loan, lender=lender)
        self.assertEqual(funding.pending_amount, Decimal("100000.00"))
        self.assertEqual(funding.amount, Decimal("0.00"))
        ...
        reviewed = self.client.post(f"/api/v1/loans/placements/{funding.id}/review/", {"approve": True}, format="json")
