...
import { ArrowLeft, Phone, Save, Search, UserRound, X } from "lucide-react-native";
...
export function ClubFormScreen() {
    ...
    return <Screen>
      ...
      {!admin ? <View style={styles.notice}><Text style={styles.noticeText}>Vous pouvez modifier uniquement le nom du club.</Text></View> : null}
      ...
      {admin ? <View style={styles.section}>
        ...
        <View style={styles.row}><View style={styles.half}><Field label="Delai de recuperation (jours)" value={form.withdrawal_notice_days} keyboardType="numeric" onChangeText={value => setForm({ ...form, withdrawal_notice_days: value })}/></View><View style={styles.half}><Field label="Co-emprunteurs max." value={form.max_collective_borrowers} keyboardType="number-pad" onChangeText={value => setForm({ ...form, max_collective_borrowers: value.replace(/[^0-9]/g, "") })}/></View></View>
      </View> : null}
