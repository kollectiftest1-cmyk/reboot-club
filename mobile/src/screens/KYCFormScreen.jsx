...
export function KYCFormScreen() {
    ...
    return <Screen>
      <PageHeader eyebrow={assisted ? "Depot assiste" : "Conformite et identite"} title={assisted ? `Dossier de ${targetUser.name}` : "Mon dossier KYC"} action={<IconButton icon={ArrowLeft} label="Retour" onPress={() => navigation.goBack()}/>}/>
      {assisted ? <View style={styles.assisted}><Text style={styles.assistedTitle}>Vous remplissez ce dossier pour {targetUser.name}</Text><Text style={styles.assistedText}>{targetUser.phone} — le membre sera notifie du depot puis de votre decision.</Text></View> : null}
