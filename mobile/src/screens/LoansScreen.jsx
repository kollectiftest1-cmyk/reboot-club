...
export function LoansScreen() {
    ...
    async function answer(invitation, accept) {
        try {
            await api(`/loans/${invitation.loan}/respond/`, { method: "POST", body: JSON.stringify({ accept, share_amount: accept && !invitation.is_replacement ? shares[invitation.id] : undefined, reason: accept ? "" : "Participation refusee par le membre" }) });
            dispatch(invalidate(["loans", "dashboard", "notifications", "validations"]));
            await load();
            setResult({ success: true, title: accept ? "Participation confirmee" : "Invitation refusee", message: invitation.is_replacement ? (accept ? "Le solde restant et les prochaines echeances vous sont maintenant attribues." : "Le participant actuel conserve sa dette.") : accept ? "Votre part est enregistree. Le dossier partira en validation apres l'accord de tous les co-emprunteurs." : "Le demandeur a ete informe et le pret collectif a ete annule." });
        } catch (error) {
    ...
    return <Screen offline={offline}>
      ...
      {invitations.length ? <View><Text style={styles.sectionTitle}>Invitations collectives</Text><View style={styles.invitationList}>{invitations.map(item => <View key={item.id} style={styles.invitation}><View style={styles.invitationTop}><View style={styles.invitationIcon}><UsersRound size={20} color={colors.forest}/></View><View style={styles.invitationCopy}><Text style={styles.invitationTitle}>{item.is_replacement ? "Reprise d'une dette" : item.purpose}</Text><Text style={styles.invitationMeta}>{item.requested_by} - {item.club_name}</Text>{item.is_replacement ? <Text style={styles.invitationMeta}>Vous remplacez {item.replaces}</Text> : null}</View><Status value="pending"/></View><View style={styles.invitationAmount}><Text style={styles.invitationAmountLabel}>{item.is_replacement ? "Capital restant propose" : "Montant total du pret"}</Text><Text style={styles.invitationAmountValue}>{money(item.is_replacement ? item.share_amount : item.loan_amount, item.currency)}</Text></View>{!item.is_replacement ? <Field label="Votre part proposee" value={shares[item.id] || ""} onChangeText={value => setShares(current => ({ ...current, [item.id]: value.replace(/[^0-9.]/g, "") }))} keyboardType="decimal-pad"/> : <Text style={styles.invitationMeta}>En acceptant, vous reprenez uniquement le solde et les echeances non payees. Les anciens paiements restent attribues au participant sortant.</Text>}<View style={styles.invitationActions}><View style={styles.invitationButton}><Button label="Refuser" variant="ghost" onPress={() => answer(item, false)}/></View><View style={styles.invitationButton}><Button label="Accepter" onPress={() => answer(item, true)}/></View></View></View>)}</View></View> : null}
