...
export function ClubMemberInviteScreen() {
    ...
    async function submit() {
        ...
        try {
            ...
            Alert.alert("Invitation envoyee", `${found.name} doit accepter l'invitation. L'administrateur effectuera ensuite la validation finale.`, [{ text: "Terminer", onPress: () => navigation.goBack() }]);
    ...
    return <Screen>
      ...
      <View style={styles.flow}>{["Le responsable envoie la demande.", "Le membre accepte depuis ses notifications.", "L'administrateur valide l'adhesion."].map((text, index) => <View key={text} style={styles.step}><Text style={styles.number}>{index + 1}</Text><Text style={styles.stepText}>{text}</Text></View>)}</View>
    <View style={styles.form}><UserRoundPlus size={24} color={colors.mintDark}/><Field icon={Phone} label="Numero du compte existant" value={phone} keyboardType="phone-pad" onChangeText={setPhone} placeholder="+243..."/><Text style={styles.label}>Profil dans le club</Text><Text style={styles.borrowerOnly}>Emprunteur uniquement. Le profil preteur est global et ne rejoint aucun club.</Text></View>
      <Button icon={Send} label="Envoyer la demande" loading={saving} disabled={phone.replace(/\D/g, "").length < 10} onPress={submit}/>
    </Screen>;
}
